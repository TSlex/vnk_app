...
namespace Webapp.ApiControllers._1._0
{
    ...
    public class AttributeTypesController : ControllerBase
    {
        ...
        public async Task<IActionResult> PatchAttributeType(long id, AttributeTypePatchDTO dto)
        {
            ...
            foreach (var valuePatchDTO in dto.Values)
            {
                AttributeTypeValue value;
                switch (valuePatchDTO.Status)
                {
                    case PatchStatus.Created:
                        await _context.TypeValues.AddAsync(new AttributeTypeValue
                        {
                    ...
                    case PatchStatus.Updated:
                        value = await _context.TypeValues.FirstOrDefaultAsync(typeValue =>
                            typeValue.Id == valuePatchDTO.Id);

                        if (value == null || value.AttributeTypeId != attributeType.Id)
                        {
                            return BadRequest(new ErrorResponseDTO("Неверный идентификатор значения"));
                        }
                    ...
                    case PatchStatus.Deleted:
                        ...
                        var attributes =
                            await _context.OrderAttributes.Where(attribute => attribute.ValueId == valuePatchDTO.Id)
                                .ToListAsync();
                    ...
                    case PatchStatus.Unchanged:
                        break;
                    default:
                        return BadRequest(new ErrorResponseDTO("Неизвестный статус обновления"));
                }
            }
            
            foreach (var unitPatchDTO in dto.Units)
            {
                AttributeTypeUnit unit;
                switch (unitPatchDTO.Status)
                {
                    case PatchStatus.Created:
                        await _context.TypeUnits.AddAsync(new AttributeTypeUnit
                        {
                            AttributeTypeId = attributeType.Id,
                    ...
                    case PatchStatus.Updated:
                        unit = await _context.TypeUnits.FirstOrDefaultAsync(typeUnit =>
                            typeUnit.Id == unitPatchDTO.Id);

                        if (unit == null || unit.AttributeTypeId != attributeType.Id)
                        {
                            return BadRequest(new ErrorResponseDTO("Неверный идентификатор значения"));
                        }
                    ...
                    case PatchStatus.Deleted:
                        ...
                        var attributes =
                            await _context.OrderAttributes.Where(attribute => attribute.UnitId == unitPatchDTO.Id)
                                .ToListAsync();
